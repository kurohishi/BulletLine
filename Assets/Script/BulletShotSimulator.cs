...
public class BulletShotSimulator : MonoBehaviour
{
    ...
    void Start()
    {
        CreatePhysicsScene(); // 開始時に仮想シーンを作る
    }
    private void Update()
    {
        Simulation(m_ballPrefab,transform.position - Vector3.up, transform.forward * _cc.bulletVellocity); // シミュレーションの結果を更新する
    }

    private Scene m_simulationScene; // 仮想シーン
    private PhysicsScene m_physicsScene; // 物理シーン
